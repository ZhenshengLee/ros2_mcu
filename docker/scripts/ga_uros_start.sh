...
function local_volumes() {
    ...
    volumes="-v $GA_ROS_ROOT_DIR:/gpuac_ws \
...
function main(){
    ...
    # zs:
    GA_CONTAINER_NAME="ga_all_${GA_DIST}_${USER}"
    ...
    # zs:
    if [ "${USER}" != "root" ]; then
        docker exec $GA_CONTAINER_NAME bash -c '/gpuac_ws/src/gpuac/gpuac_launch/scripts/docker_adduser_all.sh'
    fi

    ok "Finished setting up ga_team/ga ros environment. Now you can enter with: \nbash docker/scripts/ga_all_into.sh "
    ok "Enjoy!"
}
